...
public class ResultSetTableMetaData extends AbstractTableMetaData 
{
    ...
    throws SQLException, DataTypeException 
    {
        ...
        String databaseName = rsMetaData.getCatalogName(rsIndex);
        ...
        ResultSet columnsResultSet = metadataHandler.getColumns(databaseMetaData, databaseName, tableName);

        try
        {
            // Scroll resultset forward - must have one result which exactly matches the required parameters
            scrollTo(columnsResultSet, metadataHandler, databaseName, tableName, columnName);

            return org.dbunit.util.SQLHelper.createColumn(columnsResultSet, dataTypeFactory, true);
        }
    ...
    throws SQLException 
    {
        ...
        {
            boolean match = metadataHandler.matches(columnsResultSet,database, table, column);
            if(match)
            {
        ...
        String msg = 
                ...
                "' for <database.table> '" + database + "." + table +
                "' in database '" + database + "' because names do not exactly match.";
